(ns cmr-system-int-test.ingest-util
...
(defn update-collection
  ...
  ([provider-id collection]
   (let [full-collection (merge default-collection collection)
         ...
         response (client/put (url/collection-url {:provider-id provider-id :dataset-id (:dataset-id full-collection)})
                              {:content-type :xml
                               :body collection-xml
...
(defn delete-collection
  ...
  I call it native-id because the id in the URL used by the provider-id does not have to be the dataset id in the collection though catalog rest will enforce this."
  [provider-id native-id]
  (let [response (client/delete (url/collection-url {:provider-id provider-id :dataset-id native-id})
                                {:throw-exceptions false})
        status (:status response)]
    (is (some #{204 404} [status]))))
