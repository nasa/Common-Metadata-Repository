  ...
  (let [coll1 (d/ingest "CMR_PROV1" (dc/collection {:beginning-date-time "2000-01-01T12:00:00Z"
    ...
    (testing "search by both start-day and end-day."
      (let [t-extent "2000-02-15T00:00:00Z, 2002-03-15T00:00:00Z, 32, 90"
            references (search/find-refs :collection {"temporal[]" t-extent :page_size 100})
            references1 (search/find-refs :collection {"temporal" t-extent :page_size 100})]
    ...
    (testing "search by multiple temporal."
      (let [t-extent1 "1998-01-15T00:00:00Z, 1999-03-15T00:00:00Z, 60, 90"
            ...
            references (search/find-refs :collection
                                         {"temporal[]" [t-extent1 t-extent2]
                                          :page_size 100})
            references1 (search/find-refs :collection
                                          {"temporal" [t-extent1 t-extent2]
                                           :page_size 100})]
        (is (= references references1))
