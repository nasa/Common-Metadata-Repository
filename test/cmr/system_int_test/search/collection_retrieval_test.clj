...
(deftest retrieve-collection-by-cmr-concept-id
  (let [coll1 (d/ingest "CMR_PROV1" (dc/collection {:entry-title "Dataset1"}))]
    ...
    (testing "retrieval by collection cmr-concept-id."
      (let [response (search/retrieve-concept-metadata-by-id (:concept-id coll1))
            parsed-collection (c/parse-collection (:body response))]
        (is (= "Dataset1" (:entry-title parsed-collection)))))
    (testing "retrieval by collection cmr-concept-id, not found."
      (let [response (search/retrieve-concept-metadata-by-id "C1111-CMR_PROV1")]
