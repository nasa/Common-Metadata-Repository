...
(defn- legacy-psa-map->legacy-psa-query
  ...
  (let [{:keys [name type value minValue maxValue]} psa-map
        ...
        query (format "attribute%%5B%%5D%%5Bname%%5D=%s&attribute%%5B%%5D%%5Btype%%5D=%s" name type)]
    (if value
...
(deftest handle-legacy-psa
  (are [name type value min-value max-value]
       (let [m {:name name :type type :value value :minValue min-value :maxValue max-value}]
         (= {:attribute [(legacy-psa-map->cmr-psa-query m)]}
          (lp/process-legacy-psa {} (legacy-psa-map->legacy-psa-query m))))
