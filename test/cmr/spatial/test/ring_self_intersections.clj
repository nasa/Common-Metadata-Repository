...
(deftest ring-self-intersections-test
  (doseq [[ring-name {:keys [ords intersects]}] ring-examples]
    (let [ring (apply r/ords->ring ords)
          self-intersections (r/self-intersections ring)]
      (if intersects
        (let [sorter (fn [{:keys [lon lat]}]
                       [lon lat])
              self-intersections (distinct (sort-by sorter self-intersections))
              intersects (sort-by sorter (apply p/ords->points intersects))]
          (is (approx= self-intersections intersects 0.001)
...
(def ring-examples
  {:on-eq {:ords [0,0, 4,0, 6,5, 2,5, 0,0]}
   ...
   :around-sp2-intersects {:ords [0,-85, 75,-85, -49.68,-86.89, -166.19,-87.77,179.01,-85.86,
                           ...
                           :intersects [-49.697,-86.892, -49.656, -86.892]}
   ...
   :around-np-with-intersects {:ords [17.12,86.62, -45.66,87.6, -160.24,87.53, 164.04,86.47,
                               ...
                               :intersects [152.985,89.468]}

   :intersects-on-np {:ords [90,85, 135,85, 0,90, -135,85, -90,85, 90,85]
                      :intersects [0,90]}

   :intersects-on-np2 {:ords [90,75, 175,75, -175,90, -175,75, -90,75, 90,75]
                       :intersects [0,90]}
   ...
   :crosses-am-intersects-both-sides {:ords [-149.84,-87.66, -164.96,-86.3, 157.48,-86.11,
                                             ...
                                             -149.84,-87.66]
                                      :intersects [154.859,-86.674, -160.906,-86.822]}
   ...
   :looping-with-many-intersections {:ords [1.33,3.07, 1.94,2.84, 2.05,3.41, 0.78,3.61, 0.46,2.72,
                                            ...
                                            1.8,3.03,1.33,3.07]
                                     :intersects [2.518,3.928, 1.65,3.768, 0.793,3.608,
