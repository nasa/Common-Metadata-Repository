...
(deftest temporal-range-one-value
  (let [result (query/->cmr-temporal test-data/spatio-temporal-sentence-1)]
    (is (string/starts-with? result temporal-param))
    (is (string/includes? result "%2C")) ; comma
    (is (string/ends-with? result "Z"))
    (is (= 1
           (count-temporals result)))))

(deftest temporal-range-two-values
  (let [result (query/->cmr-temporal test-data/relative-temporals-sentence-1)]
    (is (string/starts-with? result temporal-param))
    (is (string/includes? result "%2C")) ; comma
    (is (string/ends-with? result "Z"))
    (is (= 1
