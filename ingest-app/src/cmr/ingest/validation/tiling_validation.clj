(ns cmr.ingest.validation.tiling-validation
  "Provides functions to validate the tiles during collection update"
  (:require
   [clojure.set :as s]
...
(defn deleted-tiling-searches
  "Returns granule searches for deleted tilings. We should not delete tilings in a collection
  ...
  (let [previous-tiles
    ...
    (for [name deleted-non-alias-tile-names]
      {:params {"two-d-coordinate-system[]" name
       ...
       :error-msg (format (str "Collection Tile [%s] is referenced by existing"
