...
(deftest validate-url-test
  ...
  (testing "validate url error scenarios"
    (are3 [url-value re]
      ...
      #"Invalid URL value, each S3 url must start with s://, but was http://example.com/foo"
      
      "nvalid s3 link in multiple urls"
      "s3://abc/foo,http://example.com/bar,s3://abc/baz"
      #"Invalid URL value, each S3 url must start with s://, but was http://example.com/bar")))
