  ...
  (update-in opendata-results-map [:results :dataset]
             (fn [dataset]
               (into #{} (for [field dataset]
                           ;; Dissoc title from distribtion. mimic-ingest-retrieve-metadata-conversion is returning a umm-lib
                           ;; representation after generating and parsing metadata. This is an issue with retrieving the correct
                           ;; URLContentType and will cause this test to fail since the url-content-type is not retrieved/in the umm-lib model.
