(ns cmr.system-int-test.search.collection.collection-campaign-search-test
...
;; Test collections are ingested once via setup-collections-fixture and stored in an atom.
;; This allows individual deftest blocks to be run in isolation without
;; re-ingesting data for every test, while still respecting fixture ordering constraints.
(defonce test-collections (atom nil))

(defn setup-collections-fixture [f]
  (reset! test-collections
          {:coll3 (d/ingest-umm-spec-collection "PROV1" (data-umm-c/collection {:Projects (data-umm-cmn/projects "ESI") :EntryTitle "C3" :ShortName "S3"}) {:validate-keywords false})
           :coll4 (d/ingest-umm-spec-collection "PROV2" (data-umm-c/collection {:Projects (data-umm-cmn/projects "ESI" "Esi") :EntryTitle "C4" :ShortName "S4"}) {:validate-keywords false})
           :coll5 (d/ingest-umm-spec-collection "PROV2" (data-umm-c/collection {:Projects (data-umm-cmn/projects "EVI" "EPI") :EntryTitle "C5" :ShortName "S5"}) {:validate-keywords false})
           :coll6 (d/ingest-umm-spec-collection "PROV2" (data-umm-c/collection {:Projects (data-umm-cmn/projects "ESI" "EVI" "EPI") :EntryTitle "C6" :ShortName "S6"}) {:validate-keywords false})})
  (index/wait-until-indexed)
  (f))

(use-fixtures :once
  (ingest/reset-fixture {"provguid1" "PROV1" "provguid2" "PROV2"})
  setup-collections-fixture)

(deftest search-by-single-campaign-term
...
(deftest search-by-redundant-campaign-terms
  (testing "search using redundant campaign sn terms."
  (let [{:keys [coll3 coll4 coll6]} @test-collections]
...
(deftest search-by-campaign-case-sensitivity
  (let [{:keys [coll5 coll6]} @test-collections]
...
(deftest search-by-unique-campaign-terms
...
(deftest search-by-campaign-wildcard
