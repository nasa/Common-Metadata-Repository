...
(deftest search-by-platform-humanized
  (let [_ (hu/register-humanizer)
        coll1 (d/ingest "PROV1" (dc/collection {:platforms [(dc/platform {:short-name "TERRA"})]}))
...
(deftest search-by-instrument-humanized
  (let [_ (hu/register-humanizer)
        i1 (dc/instrument {:short-name "GPS RECEIVERS"})
...
(deftest search-by-project-humanized
  (let [_ (hu/register-humanizer)
        coll1 (d/ingest "PROV1" (dc/collection {:projects (dc/projects "USGS SOFIA")}))
...
(deftest search-by-data-center-humanized
  (let [_ (hu/register-humanizer)
        coll1 (d/ingest "PROV1" (dc/collection {:organizations [(dc/org :archive-center "NSIDC")]}))
...
(deftest search-by-processing-level-id-humanized
  (let [_ (hu/register-humanizer)
        coll1 (d/ingest "PROV1" (dc/collection {:processing-level-id "1T"}))
...
(deftest search-by-science-keywords-humanized
  (let [_ (hu/register-humanizer)
        sk1 (dc/science-keyword {:category "bioosphere"
                                 :topic "topic1"
                                 :term "term1"})
