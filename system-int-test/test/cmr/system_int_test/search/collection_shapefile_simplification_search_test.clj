...
(deftest collection-shapefile-simplification-search-test
  (let [_ (side/eval-form `(shapefile/set-enable-shapefile-parameter-flag! true))
        ...
        _along-am-line (make-coll :geodetic "along-am-line"
        ...
        _touches-np (make-coll :geodetic "touches-np" (m/mbr 45 90 55 70))
        _touches-sp (make-coll :geodetic "touches-sp" (m/mbr -160 -70 -150 -90))
        _across-am-br (make-coll :geodetic "across-am-br" (m/mbr 170 10 -170 -10))
        ...
        _across-am-poly (make-coll :geodetic "across-am-poly" (polygon 170 -10, -175 -10, -170 10, 175 10, 170 -10))
        _on-np (make-coll :geodetic "on-np" (polygon 45 85, 135 85, -135 85, -45 85, 45 85))
        _on-sp (make-coll :geodetic "on-sp" (polygon -45 -85, -135 -85, 135 -85, 45 -85, -45 -85))
        ...
        _very-tall-cart (make-coll :cartesian "very-tall-cart" (polygon -160 90, -160 -90, -150 -90, -150 90, -160 90))
        ...
        _polygon-with-holes-cart (make-coll :cartesian "polygon-with-holes-cart" (poly/polygon [outer-cart hole1-cart hole2-cart]))

        ;; Points
        washington-dc (make-coll :geodetic "washington-dc" (p/point -77 38.9))
        richmond (make-coll :geodetic "richmond" (p/point -77.4 37.54))
        _north-pole (make-coll :geodetic "north-pole" (p/point 0 90))
        _south-pole (make-coll :geodetic "south-pole" (p/point 0 -90))
        _am-point (make-coll :geodetic "am-point" (p/point 180 22))
    ...
    (doseq [fmt (keys formats)
      ...
      (testing (format "Search by %s shapefile with simplification" fmt)
        (declare shapefile point-counts items)
