(ns cmr.system-int-test.search.misc.scrolling-search-test
...
(deftest scroll-id-deprecation-test
  (let [saved-flag-value (side/eval-form `(concepts-search/reject-cmr-scroll-id-flag))
        _ (side/eval-form `(concepts-search/set-reject-cmr-scroll-id-flag! true))
        expected-url (side/eval-form `(concepts-search/scroll-after-instructions-url))]
    
    (testing "CMR-Scroll-Id rejected when flag is enabled"
      (let [response (search/find-refs :collection {:scroll true}
        ...
        (is (= 400 (:status response)))
        (is (re-find #"CMR-Scroll-Id is deprecated" error-msg))
        (is (re-find #"CMR-Search-After" error-msg))
        (is (.contains (clojure.string/replace error-msg #"\s+" " ")
                       (clojure.string/replace (:body expected-url) #"\""
                                               ""))))
    ...
    (side/eval-form `(concepts-search/set-reject-cmr-scroll-id-flag! ~saved-flag-value))))
