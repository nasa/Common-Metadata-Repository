...
(defn- get-all-groups
  [context]
  (map #(get % :concept_id) (get-in (groups/search-for-groups context {}) [:results :items])))

(defn single-instance-identity-target-validation
  "Validates that the acl group exists."
  [context acl]
  (let [groups (get-all-groups context)]
    (fn [key-path target-id]
      (when-not (contains? (set groups) target-id)
...
(defn- make-single-instance-identity-validations
  ...
  [context acl]
  [{:target-id (v/when-present (single-instance-identity-target-validation context acl))}])
...
(defn- make-acl-validations
  ...
  [#(validate-provider-exists context %1 %2)
   ...
   {:single-instance-identity (v/when-present (make-single-instance-identity-validations context acl))}])
