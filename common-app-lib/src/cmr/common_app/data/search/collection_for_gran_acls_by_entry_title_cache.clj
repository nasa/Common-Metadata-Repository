...
(def cache-key
  ...
  :collections-for-gran-acls-by-entry-title)
...
(def job-refresh-rate
  "This is the frequency that the cron job will run. It should be less than coll-for-gran-acl-by-entry-title-ttl"
  ;; 15 minutes
  (* 60 15))
...
;;FIXME
(defn refresh-cache
  "Refreshes the collections-for-gran-acls collection-by-entry-title cache stored in the cache.
  ...
  [context]
  (let [collection-by-entry-title-cache (hash-cache/context->cache context cache-key)
        collections-map (fetch-collections-map context)] ;; TODO redo this logic
   (doseq [[coll-key coll-value] collections-map]
    (hash-cache/set-value collection-by-entry-title-cache collection-by-concept-id-cache-key coll-key (clj-times->time-strs coll-value)))))

(defjob RefreshCollectionsCacheForGranuleAclsByEntryTitleJob
        [ctx system]
        (refresh-cache {:system system}))

(def refresh-collections-cache-for-granule-acls-by-entry-title-job
 {:job-type RefreshCollectionsCacheForGranuleAclsByEntryTitleJob
