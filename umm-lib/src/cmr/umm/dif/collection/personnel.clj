...
(defn xml-elem->personnel
  ...
  (let [p-elems (cx/elements-at-path organization-element [:Personnel])]
    (for [pe p-elems]
      (let [roles (cx/strings-at-path pe [:Role])
...
(defn generate-personnel
  ...
  (when (not-empty personnel)
    ...
    (for [person personnel]
      (let [{:keys [roles first-name middle-name last-name emails phones addresses]} person]
        (x/element :Personnel {}
                   (for [role roles]
                     (x/element :Role {} role))
                   (when first-name
