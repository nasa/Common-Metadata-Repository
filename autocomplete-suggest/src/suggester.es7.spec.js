...
const mockClient = new es.Client({ node: 'http://foo:9200' });
...
let suggester;
beforeEach(() => {
  suggester = new ES7Suggester({ client: mockClient, index: 'autocomplete' });
});
...
test(
  ...
  () => {
    ...
    expect(mockClient.search).toBeCalledWith(expect.anything());
  },
);
...
test(
  ...
  async () => {
    // GIVEN
    mockClient.search = jest.fn().mockResolvedValue(ES7_RESPONSE_GOOD);
...
test(
  ...
  async () => {
    // GIVEN
    mockClient.search = jest.fn().mockResolvedValue(ES7_RESPONSE_EMPTY);
