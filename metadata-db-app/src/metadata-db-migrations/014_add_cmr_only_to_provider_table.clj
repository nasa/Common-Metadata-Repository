(ns migrations.014-add-cmr-only-to-provider-table
...
(defn up
  ...
  []
  (println "migrations.014-add-cmr-only-to-provider-table up...")
  (h/sql "alter table providers add cmr_only INTEGER DEFAULT 0 NOT NULL"))

(defn down
  "Migrates the database down from version 14."
  []
  (println "migrations.014-add-cmr-only-to-provider-table down.")
