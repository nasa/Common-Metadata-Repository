...
(defn up
  ...
  (doseq [t (h/get-regular-provider-collection-tablenames)]
    (h/sql (format "alter table %s add column entry_id VARCHAR(255)" t))
    (h/sql (format "update %s set entry_id = concat(short_name, '_', version_id) where format != 'DIF'" t))
    (h/sql (format "update %s set entry_id = short_name where version_id is null or format = 'DIF'" t))
    (h/sql (format "ALTER TABLE %s ALTER COLUMN entry_id SET NOT NULL" t))
