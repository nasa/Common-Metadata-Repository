...
(defmulti fields-for-find-concept
  "Returns the fields that should be included in a find-concept"
  (fn [concept-type params]
    concept-type))

(defmethod fields-for-find-concept :granule
  [concept-type params]
  (let [exclude-metatdata? (:exclude-metadata params)
        all-fields #{:provider_id :native_id :concept_id :revision_date :metadata :deleted :revision_id :format :parent_collection_id :delete_time :granule_ur}]
    (disj all-fields (when exclude-metatdata? :metadata))))

(defmethod fields-for-find-concept :collection
  [concpet-type params]
  (let [exclude-metatdata? (:exclude-metadata params)
        all-fields #{:native_id :provider_id :concept_id :revision_date :revision_id :metadata :deleted :format :entry_title :entry_id :short_name :version_id :delete_time}]
    (disj all-fields (when exclude-metatdata? :metadata))))

(defmethod fields-for-find-concept :default
  [concpet-type params]
  #{:*})
...
(defn- gen-find-concepts-in-table-sql
  ...
  (let [params (dissoc params :exclude-metadata)]
    (su/build (select fields
                (from table)
...
(defmethod find-concepts-in-table true
  ...
  (let [fields (vec (fields-for-find-concept concept-type params))
...
(defmethod find-concepts-in-table :default
  ...
  (let [provider (first providers)
        ...
        fields (vec (disj (fields-for-find-concept concept-type params) :provider_id))
