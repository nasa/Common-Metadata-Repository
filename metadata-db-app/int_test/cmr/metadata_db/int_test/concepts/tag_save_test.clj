...
(deftest save-group-with-concept-id
  (doseq [provider-id ["REG_PROV" "SMAL_PROV1"]]
    (let [serv-concept-id (str "S10-" provider-id)
          group (util/group-concept provider-id 1 {:concept-id serv-concept-id})
        ...
        (let [response (util/save-concept (assoc group :native-id "foo"))]
          (is (= {:status 409,
                  :errors [(msg/concept-exists-with-different-id
                             serv-concept-id (:native-id group)
                             serv-concept-id "foo"
                             :group provider-id) ]}
      ...
      (testing "with incorrect concept id"
        (let [other-serv-concept-id (str "S11-" provider-id)
              response (util/save-concept (assoc group :concept-id other-serv-concept-id))]
          (is (= {:status 409,
                  :errors [(msg/concept-exists-with-different-id
                             serv-concept-id (:native-id group)
                             other-serv-concept-id (:native-id group)
                             :group provider-id) ]}
                 (select-keys response [:status :errors])))))
