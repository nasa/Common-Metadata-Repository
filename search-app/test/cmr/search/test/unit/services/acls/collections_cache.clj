  ...
  (testing "round trip"
    (let [some-text (random-text)
          ...
          actual (-> supplied-data
                     coll-gran-acls-caches/time-strs->clj-times
                     coll-gran-acls-caches/clj-times->time-strs)]
...
(def get-collection-for-gran-acls #'search-coll-cache/get-collection-for-gran-acls)
...
(deftest get-collection-gran-acls-by-concept-id-test
  ...
  (testing "Testing when collection doesn't exist in cache or elastic -> Then returns nil collection"
   ...
   (with-redefs-fn {#'coll-gran-acls-caches/set-caches (fn [context coll-concept-id] nil)}
  ...
  (testing "Testing when collection cache has collection, but it is empty map in cache and elastic -> Then should find the collection in elastic, if still empty then returns empty coll"
    (with-redefs-fn {#'coll-gran-acls-caches/set-caches (fn [context coll-concept-id] {})}
  ...
  (testing "Testing when collection is not in cache, but exists in elastic -> Then should find the collection in elastic and add to cache"
    (with-redefs-fn {#'coll-gran-acls-caches/set-caches (fn [context coll-concept-id] test-coll2)}
...
;  (let [coll-by-provider-id-entry-title-cache-key coll-caches/coll-by-provider-id-and-entry-title-cache-key
...
;  (testing "Testing when collection doesn't exist in cache or elastic -> Then returns nil collection"
;   ;; mock the set-cache func
;   (with-redefs-fn {#'coll-caches/set-caches (fn [context provider-id entry-title] nil)}
;     #(is (= nil (get-collection-for-gran-acls context "NON_EXISTENT" "EntryTitle000")))))
;
;  (testing "Testing when collection cache has collection, but it is empty map in cache and elastic -> Then should find the collection in elastic, if still empty then returns empty coll"
;    (with-redefs-fn {#'coll-caches/set-caches (fn [context provider-id entry-title] {})}
;      #(is (= {} (get-collection-for-gran-acls context "TEST_PROV1" "EntryTitle1")))))
;
;  (testing "Testing when collection is not in cache, but exists in elastic -> Then should find the collection in elastic and add to cache"
;    (with-redefs-fn {#'coll-caches/set-caches (fn [context provider-id entry-title] test-coll2)}
