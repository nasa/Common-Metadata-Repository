...
(deftest create-cycle-subfacets-map-test
  ...
  (testing "cycle selected"
    (let [result (gv2f/create-cycle-subfacets-map 
      ...
      (is (= "42" (:title (first children))))
      (is (= {:remove "http://localhost:3003/granules.json?page_size=10&include_facets=v2&collection_concept_id=C1-PROV1&cycle%5B%5D=42"}
             (:links (second children)))))))
  ...
  (testing "cycle"
    (let [result (v2-facets/create-v2-facets-by-concept-type
                   ...
                   {:cycle {:buckets [{:key 3.0 :doc_count 6}
                                      {:key 42.0 :doc_count 2}]}}
                   nil)
          cycle-result (first result)]
      (println "==== the result =====")
      (clojure.pprint/pprint cycle-result)
      (is (= "Cycle" (:title cycle-result)))
      (is (= 2 (count (:children result))))))
  #_(testing "cycle and pass"
      (let [result (v2-facets/create-v2-facets-by-concept-type 
                     :granule
                     "http://localhost:3003/granules.json"
                     {"page_size" "10"
        ...
        (is (= [{:title "Cycle"          
                 ...
                 [{:title "Cycle"
                   ...
                   '({:title "3"
                      ...
                      :links
                      {:apply
                       "http://localhost:3003/granules.json?page_size=10&include_facets=v2&collection_concept_id=C1-PROV1&null%5B0%5D%5Bnull%5D=3"}
                      :has_children true}
                     {:title "42"
                      :type :filter
                      :applied false
                      ...
                      :links
                      {:apply
                       "http://localhost:3003/granules.json?page_size=10&include_facets=v2&collection_concept_id=C1-PROV1&null%5B0%5D%5Bnull%5D=42"}
                      :has_children true})}]}]
               result)))))
