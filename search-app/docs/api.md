...
##### Valid Collection Sort Keys
  ...
  * `near_current` - Sorts collection by fuzzy collection end-date in relation to near-current-days configured. Any end-date after today, minus the configured near-current-days, is considered near current.  Any end-date before that is not near current.
