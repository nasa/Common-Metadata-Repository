...
These are query parameters specific to collections
  ...
  * `include_tags` - If this parameter is set (e.g. `include_tags=gov.nasa.earthdata.search.*,gov.nasa.echo.*`), the collection results will contain an additional field 'tags' within each collection. The value of the tags field is a list of tuples of tag namespace and tag value that are associated with the collection. Only the tags with namespace matching the values of `include_tags` parameter (with wildcard support) are included in the results. This parameter is only upported in JSON result format, it has no effects on other formats.
