...
(defn- parse-visualization-urls
  "Parse visualization urls"
  [doc sanitize?]
  (for [url (select doc visualization-url-xpath)
        ;; We retrieve visualization url from two different places. This might change depending on the
        ;; outcome of ECSE-129.
        :let [visualization-url (or (value-of url "gmd:fileName/gmx:FileName/@src")
                                    (value-of url "gmd:fileName/gco:CharacterString"))
              types-and-desc (parse-url-types-from-description
                              (char-string-value url "gmd:fileDescription"))]]
     {:URL (when visualization-url (url/format-url visualization-url sanitize?))
      :Description (:Description types-and-desc)
...
(defn parse-related-urls
  ...
  (seq (concat (parse-online-and-service-urls doc sanitize?)
               (parse-visualization-urls doc sanitize?)
