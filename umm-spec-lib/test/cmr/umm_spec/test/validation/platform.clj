    ...
    (testing "invalid platforms"
      ...
      (testing "multiple errors"
        (let [coll (coll/map->UMM-C
                                    ...
                                    {:ShortName "P1"
                                     :Instruments [(c/map->InstrumentType {:ShortName "I1"
                                                   ...
                                                   (c/map->InstrumentType {:ShortName "I1"
                                                                           :ComposedOf [s1 s2 s2]})]})]})
              bad-platform (coll/map->UMM-C
                            {:Platforms [(c/map->PlatformType
                                          {:ShortName ""
                                           :LongName "¡El nombre de corto se malo!"})]})
              coll-expected-errors [{:path [:Platforms 1 :Instruments 0 :ComposedOf]
          ...
          (h/assert-multiple-invalid coll coll-expected-errors))))))
