...
describe('utils#indexCmrCollection', () => {
  describe('when an exception is thrown', () => {
    test('it is caught and logged', async () => {
      ...
      expect(consoleMock.mock.calls[0][0]).toEqual(`Error deleting project vertices only linked to collection [${conceptId}]: Cannot read property 'V' of null`)
      // Error message logged because addV failed because of null gremlinConnection
      expect(consoleMock.mock.calls[1][0]).toEqual(`Error indexing collection [${conceptId}]: Cannot read property 'addV' of null`)
    })
  })
})
