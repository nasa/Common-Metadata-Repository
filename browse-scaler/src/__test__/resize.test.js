...
describe('Image resizing', () => {
  ...
  test('Images aspect ratios are maintained when image sizes do not match', async () => {
    ...
    fs.writeFileSync('desk-flip-resize.png', imageResponse, { encoding: 'base64' });
  ...
  test('Resize image to be smaller than original, then larger than original', async () => {
    ...
    fs.writeFileSync('desk-flip-smaller.png', smallerImage, { encoding: 'base64' });
    ...
    /* Make Image Larger */
    const largerImage = await resizeImage(image, 600, 600);
    fs.writeFileSync('desk-flip-larger.png', largerImage, { encoding: 'base64' });
