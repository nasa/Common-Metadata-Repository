...
(deftest test-redis-cache-with-expire
  (testing "Redis cache with default timeout..."
    (let [rcache (redis-cache/create-redis-cache {:ttl 10000})
      ...
      (is (> (wcar* s-key true (carmine/ttl s-key)) 0)))))
  ...
  (testing "Redis cache with default timeout..."
    (let [rcache (redis-cache/create-redis-cache)
      ...
      (is (= "persist"
             (cache/get-value rcache "test")))
      (is (= -1
             (wcar* s-key true (carmine/ttl s-key)))))))

(deftest test-get-keys
  (wcar* "get-keys-pattern#-test1" false (carmine/set "get-keys-pattern#-test1" "test1"))
  (wcar* "get-keys-pattern#-test2" false (carmine/set "get-keys-pattern#-test2" "test2"))
  ...
  (testing "Get keys greater than max scan keys..."
    (doseq [x (range (* 2 (config/redis-max-scan-keys)))]
      (wcar* (str "get-keys-pattern-2#-" x) false (carmine/set (str "get-keys-pattern-2#-" x) x)))
