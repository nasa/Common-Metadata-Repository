...
(deftest read-generic-doc-file-test
  (testing
   "Reads the markdown file for the given generic if the generic is not in the system or it is the
    wrong version, return empty string"
    (is (= true (string/includes? (gdocs/read-generic-doc-file "ingest" :grid "0.0.1")
                                  "Create / Update a Grid")))
    (is (pos? (count (gdocs/read-generic-doc-file "ingest" :grid "0.0.1"))))
    (is (= true (string/includes? (gdocs/read-generic-doc-file "search" :grid "0.0.1")
                                  "Searching for Grids")))
...
;; This function is concatenating all of the generic documentation into one string to be passed into
;; the api docs
(deftest all-generic-docs-test
  (testing "This string is ensuring that all the generics have their text Concatenated together"
...
(deftest get-toc-headers-from-markdown
  ...
  (is (= (gdocs/get-toc-headers-from-markdown header-extract-full)
         (list "### <a name=\"link\"></a> content"))))
...
(deftest get-toc-data
  ...
  (is (= (gdocs/get-toc-data header-extract-formatted-search
                             "search"
                             (get options-map-search :spacer (fn [x] x)))
         "    * [content](#link)\n"))
  (is (= (gdocs/get-toc-data header-extract-formatted-ingest
                             "ingest"
                             (get options-map-ingest :spacer (fn [x] x)))
         "        * [content](#link)\n")))
...
(def formatted-search-grid-markdown
  (str "* [Grid](#grid)\n"
...
(def formatted-ingest-grid-markdown
  (str
   "    * [Grid](#grid)\n"
...
(def default-formatted-ingest-default
  (str
   "  * [Grid](#grid)\n"
   "    * [/providers/&lt;provider-id&gt;/grids/&lt;native-id&gt;](#provider-info-grid)\n"
   "   * [PUT - Create / Update a Grid](#create-update-grid)\n"
...
(deftest format-generic-toc-test
  ...
  (is (= formatted-search-grid-markdown
         (gdocs/format-generic-toc "search" "grid" "0.0.1" options-map-search)))
  (is (= formatted-ingest-grid-markdown
         (gdocs/format-generic-toc "ingest" "grid" "0.0.1" options-map-ingest)))
...
(deftest all-generic-docs-toc-test
  (testing "Ensuring that the generic-toc returns a formatted markdown")
  (is (= true (string/includes? (gdocs/all-generic-docs-toc "search" options-map-search)
                                "[Grid](#grid)"))))
