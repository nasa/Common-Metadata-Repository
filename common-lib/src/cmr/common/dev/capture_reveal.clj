(ns cmr.common.dev.capture-reveal
  "A prototype namespace that helps with debugging. It can easily capture and reveal values.
   ...
   If you use SublimeText with the typical CMR developer setup you can setup the following key
   bindings:

   // Reveal highlighted captured var value
   { \"keys\": [\"alt+super+v\"],
     \"command\": \"run_on_selection_in_repl\",
     \"args\": {\"function\": \"cmr.common.dev.capture-reveal/reveal\"}},
   { \"keys\": [\"alt+super+shift+v\"],
     \"command\": \"run_command_in_namespace_in_repl\",
     \"args\": {\"command\": \"(cmr.common.dev.capture-reveal/reveal-all)\"}},
...
cmr.common.dev.capture-reveal/reveal
  ...
  <scope>source.clojure</scope>
   ...
   of auto-completing to the text shown above.")

;; Idea for extension:
;; Consider that in sublime we like to execute blocks of code using cmd+alt+b. Often times the
;; variables for the block were passed in somewhere above that. We could capture the variables and
;; then add a new keystroke (cmd+alt+shift+b) that will execute the block with all the captured vars
;; set in an implicit let.
...
(defn capture-values
  ...
  an alternating list of symbols and their values. the-ns should be the namespace string name."
  [the-ns-name & var-sym-values]
  ...
  (let [var-sym-values-map (into {} (map vec (partition 2 var-sym-values)))]
    (swap! captured-values
           (fn [captured]
             (update-in captured [the-ns-name] merge var-sym-values-map)))))
...
(defn reveal-value
  ...
  [the-ns-name var-sym]
  (get-in @captured-values [the-ns-name var-sym]))

(defn reveal-all-values
  "Returns all the captured values for a namespace."
  [the-ns-name]
  (get @captured-values the-ns-name))
