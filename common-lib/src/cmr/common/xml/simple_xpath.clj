...
(defn- create-range-selector
  ...
  (let [[_ start-str end-str] (re-matches #"(\d+)\.\.(\d*)" selector-str)
        ...
        end-index (when-not (string/blank? end-str) (Long/parseLong end-str))]
    {:type :range-selector
...
(defn- create-tag-name-selector
  ...
  (let [tag-name (keyword (last (string/split (str tag-name) #":")))]
    {:type :tag-selector
...
(defn- create-attribute-name-selector
  ...
  ;; /attribute-name
  (let [attrib-name (keyword (string/replace (name attrib-name) #":" "/"))]
    ...
    (if-let [^String next-part (first parts)]
      (cond
        ...
        (if (.contains next-part "]")
          ...
          (recur (conj new-parts (string/join (conj joining-parts next-part)))
                 (rest parts)
                 nil)
    ...
    (case (:type selector)
      ...
      (filterv (fn [element]
                 (when-let [selected-value (first
                   ...
                   (if sub-str?
                     (string/includes? selected-value value)
...
(defn process-data-selector
  "Processes an XPath selector against clojure data (elements)."
  [data selector]
  (let [{:keys [attrib-name
                element-value
                end-index
...
;; Public API
  ...
  (defn-
    ...
    [xpath-context parsed-xpath]
    (let [{:keys [source selectors original-xpath]} parsed-xpath]
      (case (:type xpath-context)
  ...
  (defn text
    ...
    (cond
      ...
      (string/join (map text (:context context-or-node)))
      (seq? context-or-node)
      (string/join (map text context-or-node))
      (:content context-or-node)
      (string/join (map text (:content context-or-node)))))
