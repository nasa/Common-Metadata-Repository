...
(deftest non-empty-string-test
  (testing "non-empty-string test"
    (are3 [input-str expected]
          (is (= (acl-core/non-empty-string input-str) expected))
...
(deftest get-token-test
  (testing "get token test"
    (are3 [params headers expected-token]
          (is (= (acl-core/get-token params headers) expected-token))
  ...
  (testing "header token, param token exist and do not equal each other"
    (let [params {:token "params-token"}
      ...
      (is (thrown? Exception (acl-core/get-token params headers))))))
...
(deftest get-client-id-test
  (testing "get client id test"
    (are3 [headers expected-client-id]
          ...
          {"user-agent" "Mozilla."} acl-core/BROWSER_CLIENT_ID

          "user agent is curl"
          {"user-agent" "curl."} acl-core/CURL_CLIENT_ID

          "user agent is opera"
          {"user-agent" "Opera."} acl-core/BROWSER_CLIENT_ID

          "client id does not exist"
          {} acl-core/UNKNOWN_CLIENT_ID

          "unknown client id"
          {"user-agent" "Other"} acl-core/UNKNOWN_CLIENT_ID)))
...
(deftest add-authentication-to-context-test
  (let [context {}
        ...
        expected-context {:token "params-token" :client-id acl-core/CURL_CLIENT_ID}]
...
(deftest context->sids-test
  (testing "get sids from requesting it"
    ...
    (with-redefs [cmr.acl.core/request-sids (fn [_context] [:guest])]
      (is (= (acl-core/context->sids context) [:guest]))))
...
(deftest get-permitting-acls-test
  (testing "Exception getting acls"
    (with-redefs [cmr.transmit.access-control/acl-type->acl-key 
                  (fn [_object-identity-type] 
                    (throw (Exception. "Exception to break test")))]
      (is (nil? (acl-core/get-permitting-acls nil nil nil nil)))))
  (testing "Exception getting acls with 401 message"
    (with-redefs [cmr.transmit.access-control/acl-type->acl-key 
                  (fn [_object-identity-type] 
                    (throw (Exception. "Exception to break test with status 401")))]
      (is (thrown? Exception (acl-core/get-permitting-acls nil nil nil nil))))))
